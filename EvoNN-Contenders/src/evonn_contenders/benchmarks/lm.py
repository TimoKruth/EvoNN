"""Language-modeling helpers for contender benchmarks."""
    ...
    *,
    ...
    for idx in range(n_samples):
        ...
        if mode == 0:
            ...
            full = np.asarray([a if pos % 2 == 0 else b for pos in range(context_length + 1)], dtype=np.int32)
        else:
...
def resolve_lm_cache_path(dataset: str) -> Path:
    ...
    env_root = os.environ.get("EVONN_CONTENDERS_LM_CACHE_DIR")
    roots: list[Path] = []
    if env_root:
