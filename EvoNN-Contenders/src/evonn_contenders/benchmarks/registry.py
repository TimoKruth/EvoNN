"""Dataset registry and loaders backed by contender-owned benchmark catalog."""
...
from evonn_contenders.benchmarks.lm import generate_synthetic_lm_dataset, load_cached_lm_dataset
...
_CACHE_DIR = Path.home() / ".evonn-contenders" / "data"
...
class DatasetRegistry:
    """Resolve benchmark metadata and load arrays."""
    ...
    def _fetch_sklearn(self, meta: DatasetMeta) -> tuple[np.ndarray, np.ndarray]:
        ...
        spec_path = _resolve_catalog_dir() / f"{meta.name}.yaml"
    ...
    def _load_csv_file(self, path: Path, meta: DatasetMeta) -> tuple[np.ndarray, np.ndarray]:
        ...
        x = np.asarray([[float(row[column]) for column in feature_cols] for row in rows], dtype=np.float32)
        if meta.task == "classification":
