"""Benchmark spec model and loading helpers."""
...
from evonn_contenders.benchmarks.lm import (
...
class BenchmarkSpec(BaseModel):
    ...
    def load_data(
        ...
        *,
        ...
        """Load small datasets for supported sources."""
        ...
        if self.source in {"openml", "image"}:
            from evonn_contenders.benchmarks.registry import DatasetRegistry

            return DatasetRegistry().load_data(self.dataset or self.name, seed=seed, validation_split=validation_split)
        ...
        raise NotImplementedError(f"Source '{self.source}' metadata available, full loader not implemented yet")
    ...
    def _load_sklearn(self, *, seed: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        if self.dataset == "make_circles":
            return skd.make_circles(n_samples=self.n_samples, noise=self.noise, factor=self.factor, random_state=seed)
