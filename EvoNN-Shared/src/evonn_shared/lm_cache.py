...
CACHE_FORMAT = "evonn_byte_lm_cache_v1"
REQUIRED_CACHE_ARRAYS = ("x_train", "y_train", "x_val", "y_val")
...
def validate_lm_cache(path: Path | str) -> dict[str, object]:
    ...
    with np.load(cache_path, allow_pickle=False) as payload:
        missing = [name for name in REQUIRED_CACHE_ARRAYS if name not in payload.files]
        if missing:
            return {
                "path": str(cache_path),
                "ok": False,
        ...
        arrays = {name: payload[name] for name in REQUIRED_CACHE_ARRAYS}
        metadata = _decode_metadata(payload)

    blockers: list[str] = []
    warnings: list[str] = []
    for split in ("train", "val"):
        _validate_split_arrays(split, arrays[f"x_{split}"], arrays[f"y_{split}"], blockers)
    ...
    if metadata.get("format") != CACHE_FORMAT:
        warnings.append(f"cache metadata missing or not generated by {CACHE_FORMAT}")
...
def _linear_successor_fraction(arrays: dict[str, np.ndarray]) -> float:
    ...
    combined = np.concatenate(matches)
...
def _next_token_alignment_fraction(arrays: dict[str, np.ndarray]) -> float:
    ...
    combined = np.concatenate(matches)
    return float(np.mean(combined)) if combined.size else 1.0
